...
void pause(uint16_t ms) {
  ...
  delayMillis(ms);
  TIM1->CCR1 = beep_volume; // volume of the beep, (duty cycle) don't go above
                            // 25 out of 2000
...
void setVolume(uint8_t volume) {
  ...
  beep_volume =
      volume * 2; // volume variable from 0 - 11 equates to CCR value of 0-22
}
...
void playBJNote(uint16_t freq, uint16_t bduration) { // hz and ms
  ...
  TIM1->CCR1 = beep_volume * timerOne_reload /
               TIM1_AUTORELOAD; // volume of the beep, (duty cycle) don't go
