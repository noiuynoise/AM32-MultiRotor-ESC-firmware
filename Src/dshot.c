...
void computeDshotDMA() {
  ...
  if ((dshot_frametime < 1350) && (dshot_frametime > 1150)) {
    for (int i = 0; i < 16; i++) {
      dpulse[i] =
          ((dma_buffer[j + (i << 1) + 1] - dma_buffer[j + (i << 1)]) >> 5);
    }
#endif
    ...
    if ((dshot_frametime < 1690) && (dshot_frametime > 1600)) {
      for (int i = 0; i < 16; i++) {
        dpulse[i] =
            ((dma_buffer[j + (i << 1) + 1] - dma_buffer[j + (i << 1)]) >> 6);
      }
#endif
#if defined(MCU_L431)
      if ((dshot_frametime < 1800) && (dshot_frametime > 1650)) {
        ...
        if (!armed) {
          if (dshot_telemetry == 0) {
            if (INPUT_PIN_PORT->IDR &
                INPUT_PIN) { // if the pin is high for 100 checks between signal
                             // pulses its inverted
              high_pin_count++;
        ...
        int tocheck =
            (dpulse[0] << 10 | dpulse[1] << 9 | dpulse[2] << 8 |
             dpulse[3] << 7 | dpulse[4] << 6 | dpulse[5] << 5 | dpulse[6] << 4 |
             dpulse[7] << 3 | dpulse[8] << 2 | dpulse[9] << 1 | dpulse[10]);
    ...
    void make_dshot_package() {
      if (send_extended_dshot > 0) {
        ...
        //	calculate shift amount for data in format eee mmm mmm mmm, first
        // 1 found in first seven bits of data determines shift amount
        ...
        dshot_full_number =
            ((shift_amount << 9) | (e_com_time >> shift_amount));
      }
      // calculate checksum
      uint16_t csum = 0;
      ...
      dshot_full_number =
          (dshot_full_number << 4) |
          csum; // put checksum at the end of 12 bit dshot number

      // GCR RLL encode 16 to 20 bit

      gcrnumber =
          gcr_encode_table[(dshot_full_number >> 12)]
          ...
          | gcr_encode_table[(((1 << 4) - 1) &
                              (dshot_full_number >> 0))]; // last four digits
// GCR RLL encode 20 to 21bit output
      ...
      for (int i = 19; i >= 0; i--) { // each digit in gcrnumber
        gcr[buffer_padding + 20 - i + 1] =
            ((((gcrnumber & 1 << i)) >> i) ^
             (gcr[buffer_padding + 20 - i] >> 6))
            << 6; // exclusive ored with number before it multiplied by 64 to
                  // match output timer.
      }
      ...
      for (int i = 19; i >= 0; i--) { // each digit in gcrnumber
        gcr[7 + 20 - i + 1] =
            ((((gcrnumber & 1 << i)) >> i) ^ (gcr[7 + 20 - i] >> 6)) *
            94; // exclusive ored with number before it multiplied by 64 to
                // match output timer.
      }
      gcr[7] = 0;
#endif
      ...
      for (int i = 19; i >= 0; i--) { // each digit in gcrnumber
        gcr[7 + 20 - i + 1] =
            ((((gcrnumber & 1 << i)) >> i) ^ (gcr[7 + 20 - i] >> 6)) *
            84; // exclusive ored with number before it multiplied by 64 to
                // match output timer.
      }
      gcr[7] = 0;
#endif
