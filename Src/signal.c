...
void computeMSInput() {
  ...
  for (int j = 1; j < 2; j++) {
    ...
    if (((dma_buffer[j] - lastnumber) < 1500) &&
        ((dma_buffer[j] - lastnumber) > 0)) { // blank space
...
void computeServoInput() {
  ...
  if (((dma_buffer[1] - dma_buffer[0]) > 800) &&
      ((dma_buffer[1] - dma_buffer[0]) < 2200)) {
    if (calibration_required) {
      if (!high_calibration_set) {
        ...
        if (getAbsDif(last_high_threshold, servo_high_threshold) > 50) {
          ...
          servo_high_threshold =
              ((7 * servo_high_threshold + (dma_buffer[1] - dma_buffer[0])) >>
               3);
      ...
      if (high_calibration_set) {
        if (dma_buffer[1] - dma_buffer[0] < 1250) {
          ...
          servo_low_threshold =
              ((7 * servo_low_threshold + (dma_buffer[1] - dma_buffer[0])) >>
               3);
        }
      ...
      if (bi_direction) {
        if (dma_buffer[1] - dma_buffer[0] <= servo_neutral) {
          ...
          servorawinput =
              map((dma_buffer[1] - dma_buffer[0]), servo_neutral + 1,
                  servo_high_threshold, 1001, 2000);
        }
      } else {
        servorawinput =
            map((dma_buffer[1] - dma_buffer[0]), servo_low_threshold,
  ...
  if (inputSet == 1) {
    ...
    if (dshot_telemetry) {
      if (out_put) {
        ...
        receiveDshotDma();    // holy smokes.. reverse the line and set up dma
                              // again
        return;
      } else {
        sendDshotDma();
    ...
    if (!armed) {
      ...
      if (adjusted_input == 0 &&
          calibration_required == 0) { // note this in input..not newinput so it
