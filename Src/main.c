...
void checkForHighSignal() {
  ...
  for (int i = 0; i < 1000; i++) {
    if (!(INPUT_PIN_PORT->IDR &
          INPUT_PIN)) { // if the pin is low for 5 checks out of 100 in  100ms
                        // or more its either no signal or signal. jump to
                        // application
      low_pin_count++;
    }
    delayMicros(10);
...
void interruptRoutine() {
  if (average_interval > 125) {
    if ((INTERVAL_TIMER->CNT < 125) && (duty_cycle < 600) &&
    ...
    if (stepper_sine == 0) {
      ...
      if (input > 48 && armed) {
        ...
        if (input > 48 && input < 137) { // sine wave stepper
          ...
          if (do_once_sinemode) {
            COM_TIMER->DIER &=
                ~((0x1UL << (0U))); // disable commutation interrupt in case set
            maskPhaseInterrupts();
            TIM1->CCR1 = 0;
            TIM1->CCR2 = 0;
            TIM1->CCR3 = 0;
